...
| **根因** | **path 3 内部非确定性** | path 3 单独运行仍不一致，非 path 1/2 污染 |

### 关键结论
- **问题不在** gHammingMatch 算法（隔离测试：同一输入 → 输出一致）
- **问题不在** path 1/2 污染（path_order_inv、path3_only 均不一致）
- **问题在** path 3 内部：detectAndCompute 与 cuMatch 之间，或 cuMatch 在「紧接 detectAndCompute 后」执行时的非确定性
...
4. **排查 path 3 内部**：detectAndCompute 与 cuMatch 之间加 `cudaDeviceSynchronize`/`cudaMemset`，或检查 detectAndCompute 残留状态

### 关键文件
| 文件 | 用途 |
|------|------|
| `repro_test.cpp` | 完整 repro 测试，支持 `--dump-match-input`、第三参数 `path_order_inv`/`path3_only` |
| `run_reproducibility_test.sh` | 32 次 repro_test，比较入参和输出，并做 gHammingMatch 隔离测试 |
| `run_path_order_tests.sh` | path_order_inv + path3_only 各 32 次，验证 path 1/2 是否污染 |
...
**结论**：path 1/2 **不是**污染源。path 3 单独运行或先运行时，match/dist 仍不一致，说明非确定性来自 **path 3 内部**（detectAndCompute 与 cuMatch 之间，或 cuMatch 在 detectAndCompute 之后执行时的行为）。
...
检查 gHammingMatch 的 `__shared__` 未初始化读取。**需 Compute Sanitizer 2025.4+**，当前 2025.1 会报 `unrecognised option '--initcheck-address-space'`。升级 CUDA 后可执行此检查。
...
4. **detectAndCompute 与 cuMatch 之间**：在 path 3 内、cuMatch 之前插入 `cudaDeviceSynchronize()`，或对 data1.d_data/data2.d_data 中 match/dist 区域做 `cudaMemset`  
5. **initcheck shared**：`--initcheck-address-space shared` 检查 gHammingMatch 的 `__shared__` 未初始化读取
