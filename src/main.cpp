...
void drawKeypoints(akaze::AkazeData& a, cv::Mat& img, cv::Mat& dst, const double alpha)
{
	...
	for (int i = 0; i < a.num_pts; i++)
	{
		...
		cv::circle(dst, center, std::max(1, std::min(5, (int)p.size)), color);
	}
}
...
void cudaAkazeDemo2(int argc, char** argv)
{
	...
	{
		limg = cv::imread("test/image.png", cv::IMREAD_GRAYSCALE);
		rimg = cv::imread("test/template.png", cv::IMREAD_GRAYSCALE);
	}
