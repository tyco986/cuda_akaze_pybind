...
#### 1. initcheck（未初始化内存）
- **状态**：检测到 **219200 个错误**
- **位置**：`akaze::Akazer::detect` → `detectAndCompute`，`cudaMemcpy` 的 **source 为未初始化主机内存**
- **调用栈**：`detect` → `detectAndCompute` → `main`
- **说明**：Host API 在 cudaMemcpy 时发现 source 缓冲区（约 368640 字节）存在未初始化访问。可能与 `allocMemory` 分配的 `tmem` 或中间 buffer 有关。程序最终因 `cudaErrorLaunchTimeout` 超时退出（Sanitizer 显著拖慢执行）。

#### 2. racecheck（共享内存数据竞争）✅ 关键发现
...
### 当前限制
- initcheck 运行较慢，可能触发 `cudaErrorLaunchTimeout`，可考虑 `CUDA_DEVICE_ORDER=PCI_BUS_ID` 或缩短测试
...
6. **initcheck 未初始化内存**：排查 `detect` 中 cudaMemcpy 的 host source 是否已正确初始化
